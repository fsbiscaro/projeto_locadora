...
if($meuToken->validarToken($authorization)==true){
    ...
    if($objEmprestimo->getValor_emprestimo()==""){
        $objResposta->cod = 1;
        $objResposta->msg = "Valor do empréstimo não pode ser vazio";
        $objResposta->status = false;
    }else{
        if($objEmprestimo->create()==true){
            $objResposta->cod = 1;
            $objResposta->msg = "Empréstimo cadastrado com sucesso";
            $objResposta->status = true;
            $objResposta->emprestimo = $objEmprestimo;
        }else{
            $objResposta->cod = 2;
            $objResposta->msg = "Erro ao cadastrar empréstimo";
            $objResposta->status = false;
        }
    }
